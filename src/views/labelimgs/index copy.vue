...
const images = ref([
  {
    imageUrl:
      "https://zjut-anomalies-source.oss-cn-hangzhou.aliyuncs.com/15210A-Bin-041.png",
    annotationUrl:
      "https://zjut-anomalies-source.oss-cn-hangzhou.aliyuncs.com/15210A-Bin-041.txt" // 可以在这里添加标注URL
  }
  // {
...
const nextImage = () => {
  ...
  console.log(
    `切换到图片${currentImageIndex.value + 1}，该图片有${rectangles.value.length}个矩形`
  );
};
...
const getImageBounds = () => {
  ...
  const containerRect =
    imageElement.value.parentElement.getBoundingClientRect();
...
const finishDrawing = event => {
  ...
  if (width > 0.01 && height > 0.01) {
    ...
    console.log(
      `✅ 保存完成，图片${currentImageIndex.value + 1}现有${newRects.length}个矩形`
    );
  } else {
...
const handleToolbarClick = (item, index) => {
  ...
  switch (item.label) {
    ...
    case "保存":
      ...
      console.log(
        `当前图片${currentImageIndex.value + 1}的矩形:`,
        rectangles.value
      );
      ...
      // 统计信息
      const totalRects = Object.values(allRectangles.value).reduce(
        (sum, rects) => sum + rects.length,
        0
      );
      console.log(
        `总计: ${Object.keys(allRectangles.value).length}张图片，${totalRects}个矩形`
      );
      item.active = true;
      break;
    // 可以根据需要添加更多功能
...
onMounted(() => {
  ...
  console.log(
    "快捷键已启用: A(上一张) D(下一张) W(绘制模式) 0-5(类型切换) Del(删除悬停矩形)"
  );
});
...
<template>
  ...
  <div class="flex bg-gray-100 absolute w-full">
      ...
      <div class="flex-1 overflow-y-auto">
        ...
        >
          <div class="flex flex-col items-center text-xs">
            ...
            <span class="text-center text-gray-600 leading-tight">{{
              item.label
            }}</span>
          </div>
        </div>
    ...
    <!-- 中间图像渲染区 (70%) -->
    <div
      class="w-[70%] bg-black relative flex items-center justify-center overflow-hidden"
    >
      <!-- 图像容器 -->
        ...
        <img
          ...
          @load="
            async () => {
              ...
              if (
                currentImageData?.annotationUrl &&
                !allRectangles.value[currentImageIndex.value]
              ) {
                try {
                  ...
                  console.log(
                    `加载图片${currentImageIndex.value + 1}的YOLO标注:`,
                    rectangles
                  );
                } catch (error) {
            ...
            <rect
              ...
              :fill="
                hoveredRectId === rect.id
                  ? 'rgba(255, 255, 0, 0.4)'
                  : getClassColor(rect.classId).fill
              "
              :stroke="
                hoveredRectId === rect.id
                  ? '#fbbf24'
                  : getClassColor(rect.classId).stroke
              "
      ...
      >
        总计:
        {{
          Object.values(allRectangles).reduce(
            (sum, rects) => sum + rects.length,
            0
          )
        }}个矩形
      ...
      <!-- 图像导航控制 -->
      <div
        class="absolute top-4 left-1/2 transform -translate-x-1/2 bg-black bg-opacity-50 text-white px-3 py-1 rounded text-sm"
      >
      ...
      >
        X: {{ Math.round(mouseX * 1000) / 1000 }}; Y:
        {{ Math.round(mouseY * 1000) / 1000 }}
      </div>
    </div>
    ...
    <div class="w-[20%] bg-white border-l border-gray-300 flex flex-col">
      ...
      <!-- 矩形列表 -->
      <div v-if="rectangles.length > 0" class="p-3 border-b border-gray-200">
        <h4 class="text-sm font-medium mb-2">
          当前图片的矩形 ({{ rectangles.length }})
          <span class="text-xs text-gray-500"
            >- 图像{{ currentImageIndex + 1 }}</span
          >
        </h4>
        <div class="space-y-1 max-h-32 overflow-y-auto">
          ...
          >
            <div class="flex items-center">
              <span
                :class="
                  hoveredRectId === rect.id ? 'font-bold text-yellow-800' : ''
                "
              >
                矩形 {{ index + 1 }}
        ...
        <div
          v-if="hoveredRectId"
          class="mt-2 text-xs text-yellow-700 bg-yellow-50 p-2 rounded"
        >
          💡 按
          <kbd class="bg-yellow-200 px-1 rounded">Del</kbd> 键删除悬停的矩形
        </div>
      </div>
      ...
      <div class="flex-1 p-3 overflow-y-auto">
        ...
        <div class="space-y-1">
          <div
            ...
            :class="{
              'bg-blue-100 text-blue-800': index === currentImageIndex
            }"
            @click="
          ...
          >
            <div class="flex justify-between items-center">
              ...
              <span>{{
                images[index + 1] && images[index + 1]["imageUrl"]
                  ? images[index + 1]["imageUrl"].split("/").pop()
                  : ""
              }}</span>

              <div class="flex items-center space-x-1">
                <span
                  v-if="image.annotationUrl"
                ...
                <span
                  v-if="allRectangles[index]?.length"
